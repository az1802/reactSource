...
let runIdCounter: number = 0;//运行ID计数
let mainThreadIdCounter: number = 0;//主线程ID计数

const profilingStateSize = 4;
// 根据性能是否开启来赋值sharedProfilingBuffer  使用ArrayBuffer 可以精细的控制内容,每个元素控制大小为32位。
// SharedArrayBuffer便于不同的线程可以共享这一块的内容否则就需要在不同的线程之间对内容进行复制会有一定的性能损耗,
// 但是这种就回产生多个线程同时使用同一块的内存内容
...
if (enableProfiling) {
  ...
  // 维护计数器优先级的队列中可能包含取消的任务
  profilingState[QUEUE_SIZE] = 0;
  profilingState[CURRENT_TASK_ID] = 0;
}

// Bytes per element is 4  每个元素的字节都是4 
const INITIAL_EVENT_LOG_SIZE = 131072;
const MAX_EVENT_LOG_SIZE = 524288; // Equivalent to 2 megabytes 2的20次方
