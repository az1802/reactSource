  ...
  for (const propKey in nextProps) {
    ...
    if (propKey === STYLE) {
      ...
      if (typeof nextProp === 'string') {
        ...
        // IE11textarea设置空字符串会导致placeholder不可见
...
export function setInitialProperties(
  ...
  // 针对一些特殊性的媒体组件犹豫事件不会向上传播所以此处不能进行事件委托必须在初始化的时候进行事件的绑定
  let props: Object;
  switch (tag) {
    case 'iframe':
    case 'object':
...
// Calculate the diff between the two objects.  计算两个属性的差异
export function diffProperties(
  ...
  for (propKey in lastProps) {
    ...
    if (propKey === STYLE) { //style对象的处理  这里是全部放入styleUpdates对象并置空
  ...
  // 处理新属性  所有的更新操作都会保存到updatePayload数组中
