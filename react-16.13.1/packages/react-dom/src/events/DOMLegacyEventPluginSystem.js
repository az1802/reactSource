...
function handleTopLevel(bookKeeping: BookKeepingInstance) {
  ...
  let ancestor = targetInst; //fiber节点
  do {
    if (!ancestor) {
...
export function legacyListenToEvent(
  ...
  const listenerMap = getListenerMapForElement(mountAt); //已经绑定的事件类型
  const dependencies = registrationNameDependencies[registrationName]; //获取该事件注册的事件依赖

  for (let i = 0; i < dependencies.length; i++) {
    const dependency = dependencies[i];
    // 将依赖的事件注册到mountAt DOM节点上
...
export function legacyListenToTopLevelEvent(
  ...
  if (!listenerMap.has(topLevelType)) { //该事件类型还未委托处理到dom节点上
    switch (topLevelType) {
      case TOP_SCROLL:
