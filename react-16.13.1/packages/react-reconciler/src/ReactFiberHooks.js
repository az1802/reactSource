...
export function renderWithHooks(
    ...
    // 执行render函数得到新的children
    let children = Component(props, secondArg);

    // Check if there was a render phase update   render阶段的更新
...
function mountWorkInProgressHook(): Hook {
    ...
    if (workInProgressHook === null) {
        // This is the first hook in the list  第一次调用hook函数
        currentlyRenderingFiber.memoizedState = workInProgressHook = hook;
    } else {
        // Append to the end of the list 重新运行hook函数
...
function mountState<S>(
    ...
    const dispatch: Dispatch<
        ...
        > = (queue.dispatch = (dispatchAction.bind(
            null,
            currentlyRenderingFiber,//当前处理的fiber节点
            queue,
        ): any));
    return [hook.memoizedState, dispatch];
}
...
/**
 * 创建更新对象添加到fiber节点的更新对额中,action为新的数据,最后调度更新
...
function dispatchAction<S, A>(
    ...
    if (__DEV__) {
        if (typeof arguments[3] === 'function') {//useState返回的更新值的方法不支持传入方法,需要传入新的值
            console.error(
