...
// Whether to root completed, errored, suspended, etc. 根节点退出的状态
let workInProgressRootExitStatus: RootExitStatus = RootIncomplete;
// A fatal error, if one is thrown  致命错误
...
let hasUncaughtError = false; //有未捕获的错误
let firstUncaughtError = null;//第一个未补货的错误
...
export function requestCurrentTimeForUpdate() {
  if ((executionContext & (RenderContext | CommitContext)) !== NoContext) {
    ...
    // 在react render 和 commit的过程之中
    return msToExpirationTime(now());
  }
  // We're not inside React, so we may be in the middle of a browser event.
  // 在浏览器的事件之中不在react的渲染之中
  if (currentEventTime !== NoWork) {
    // Use the same start time for all updates until we enter React again.
    // 表明当前有交互事件正在处理 返回同一个起始的时间 相当于变相提升了优先级
...
// on a fiber.
// 有更新行为的fiber节点,expirationTime为就算出的过期时间  往上递归处理父级节点的expirationTime
function markUpdateTimeFromFiberToRoot(fiber, expirationTime) {
  // Update the source fiber's expiration time  更新为优先级更高的事件
...
function performConcurrentWorkOnRoot(root, didTimeout) {
  ...
  if (expirationTime !== NoWork) {
    ...
    if (workInProgress !== null) {
      ...
      // 在handleError中workInProgressRootExitStatus会被赋值
...
function performSyncWorkOnRoot(root) {
  ...
  if (root !== workInProgressRoot || expirationTime !== renderExpirationTime) {
    prepareFreshStack(root, expirationTime); //准备执行任务栈即初始化workInProgress以及一些全局变量
    startWorkOnPendingInteractions(root, expirationTime); //开始处理那些待处理的交互
  }

  // If we have a work-in-progress fiber, it means there's still work to do
  // in this root.
  // 如果workInProgress不为null表示有任务需要处理
  if (workInProgress !== null) {
    const prevExecutionContext = executionContext;
    executionContext |= RenderContext; //开始render阶段
    // 之前的调度者和之前暂停的工作
    const prevDispatcher = pushDispatcher(root);
    const prevInteractions = pushInteractions(root);
    // 开始workInProgress(即被调度的fiber node)  主要是做一些标记性的工作
    startWorkLoopTimer(workInProgress);

    do {
      try {
        workLoopSync(); //开始根据根节点递归的处理子级节点执行render阶段
        break;
      } catch (thrownValue) {
        // 即使中间组件有错误抛出内部的workLoopSync还是会在错误抛出中断之后继续运行
        handleError(root, thrownValue);
      }
    } while (true);
    ...
    // TODO 渲染完成之后workInProgress根节点会置为null等待下次重新更新
    if (workInProgress !== null) {
      ...
      stopFinishedWorkLoopTimer();
      // 将workInProgress根节点挂载到root的finishedWork上
...
export function batchedUpdates<A, R>(fn: A => R, a: A): R {
  const prevExecutionContext = executionContext;
  executionContext |= BatchedContext; //执行上下文中加入批量更新
  try {
    return fn(a);
  } finally {
    executionContext = prevExecutionContext;
...
function prepareFreshStack(root, expirationTime) {
  ...
  //复制一个fiber节点key为空
  workInProgress = createWorkInProgress(root.current, null); //根fiber节点的属性时固定的所以后续不存在更新
  ...
  do {
    try {
      ...
      if (workInProgress === null || workInProgress.return === null) {
        ...
        workInProgress = null; //根节点则后续没有fiber节点需要处理
        return null;
      }
      ...
      // 节点排除异常之后fiber节点会打赏InComplete的副作用标签表明产生错误
      throwException(
        root,
      ...
      // 当前节点错误则标记为完成状态不需要处理其子节点 
      workInProgress = completeUnitOfWork(workInProgress);
    } catch (yetAnotherThrownValue) {
      // Something in the return path also threw.
      thrownValue = yetAnotherThrownValue;
      continue;//表明仍将继续循环直到没有错误抛出
    }
...
function performUnitOfWork(unitOfWork: Fiber): Fiber | null {
  ...
  if (next === null) {
    ...
    // 没有next表示没有下一个工作单元 当前的处理节点需要处理则标记为完成
...
function completeUnitOfWork(unitOfWork: Fiber): Fiber | null {
  ...
  // 当前fiber节点标记完成状态 然后递归处理其下一个兄弟节点
  ...
  do {
    ...
    // Check if the work completed or if something threw. 检查节点是否有错误抛出
    if ((workInProgress.effectTag & Incomplete) === NoEffect) {
      ...
      resetCurrentDebugFiberInDEV();
      // 重置子节点的ExpirationTime因为当前的子树节点有fiber节点完成所有当前节点的child ExpirationTime可能会发生变化
      resetChildExpirationTime(workInProgress);

      // 找到下一个待处理的fiber节点则返回 否则寻找其兄弟节点不存在则网上层递归父节点的兄弟节点一次类推
      if (next !== null) {
        // Completing this fiber spawned new work. Work on that next.
        return next;
      }

      // 父节点所有子节点处理完毕了之后就回把所有子节点的effect 挂载到父节点上
      if (
        returnFiber !== null &&
        ...
        // at the end.
        // 如果当前节点有副作用把当前节点的effect也挂载到父节点上 前面只处理了子节点
      ...
      if (next !== null) {
        ...
        // from the effect tag.
        // TODO: The name stopFailedWorkTimer is misleading because Suspenseyouku
  ...
  // We've reached the root.  如果是根节点则整棵树构造完成
  if (workInProgressRootExitStatus === RootIncomplete) {
...
// 当子树下有fiber节点状态变为完成 则需要更新fiber节点的Child ExpirationTime
function resetChildExpirationTime(completedWork: Fiber) {
  ...
  if (enableProfilerTimer && (completedWork.mode & ProfileMode) !== NoMode) {
    ...
    let child = completedWork.child;
    // 遍历未完成的子节点找到优先级最高的ExpirationTime
    while (child !== null) {
      const childUpdateExpirationTime = child.expirationTime;
      const childChildExpirationTime = child.childExpirationTime;
      if (childUpdateExpirationTime > newChildExpirationTime) {
...
function commitRootImpl(root, renderPriorityLevel) {
  ...
  // 提取完成的workInProgress树进行commit操作
  const finishedWork = root.finishedWork;
  const expirationTime = root.finishedExpirationTime;
  if (finishedWork === null) { //没有需要处理的effects
    return null;
  }
  root.finishedWork = null;
  root.finishedExpirationTime = NoWork;
  ...
  let firstEffect;
  if (finishedWork.effectTag > PerformedWork) { // 当前的根节点带effect 这个也加到effect连中
  ...
  if (firstEffect !== null) {
    ...
    // 调换fiber树将workInProgress树更新到root fiber的current上
  ...
  while (nextEffect !== null) {
    ...
    if ((effectTag & Passive) !== NoEffect) {
      ...
      if (!rootDoesHavePassiveEffects) {
        ...
        // 调度运行flushPassiveEffects  副作用的操作
...
// 检查嵌套更新的次数超过50则发出警告(在willupdate didupdate中又执行了数据的更新操作)
function checkForNestedUpdates() {
  ...
  // 在useEffect中又使用了setState 导致了递归更新进入死循环
  if (__DEV__) {
...
if (__DEV__ && replayFailedUnitOfWorkWithInvokeGuardedCallback) {
  let dummyFiber = null; //复制的fiber node
  beginWork = (current, unitOfWork, expirationTime) => {
    ...
    // 复制fiber节点 当beginWork方法运行出错时可以对fiber节点的信息进行回退
...
// 将挂起的交互行为根据expirationTime注册到pendingInteractionMap中方便获取
...
function startWorkOnPendingInteractions(root, expirationTime) {
  ...
  // 提取前面挂起的那些交互行为 然后添加到interactions中
