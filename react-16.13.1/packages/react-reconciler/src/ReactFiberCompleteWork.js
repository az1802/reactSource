...
// 增加一个update effectTag的标记
function markUpdate(workInProgress: Fiber) {
  // Tag the fiber with an update effect. This turns a Placement into
  // a PlacementAndUpdate.
  workInProgress.effectTag |= Update;
}
// 增加一个ref effectTag的标记
...
if (supportsMutation) {
  ...
  // needsVisibilityToggle   dom元素是否可见  isHidden  dom元素是否隐藏
  appendAllChildren = function(
    ...
    while (node !== null) {
      if (node.tag === HostComponent || node.tag === HostText) {
        ...
        // the portal directly.
      } else if (node.child !== null) {//处理子节点
        node.child.return = node;//TODO 放置节点为建立联系 添加子节点不成功
        node = node.child;
        continue;
      }
      ...
      while (node.sibling === null) {//前面循环向下进入到了子节点的处理 此处循环向上返回到父节点
  ...
  updateHostComponent = function(
    ...
    // 需要更新的属性赋值到updateQueue后续更新会依照这些数据进行DOM节点的更新
    workInProgress.updateQueue = (updatePayload: any);
    // If the update payload indicates that there is a change or if there
    // is a new ref we mark this as an update. All the work is done in commitWork.
    if (updatePayload) { //存在更新则在fiber节点上标记uodate Effect tag
...
//fiber节点为完成状态 相当于开始工作时的反向流程内部会pop一些值即出栈
// 文本节点会直接在此处创建textDOM节点
function completeWork(
  ...
  switch (workInProgress.tag) {
    ...
    case HostComponent: {
      ...
      if (current !== null && workInProgress.stateNode != null) {//是否是第一次渲染或者第一次挂载
        // 更新组件需要更新的DOM属性并且将更新内容放到fiber节点的updateQueue上
        updateHostComponent(
          current,
          workInProgress,
          type,
        ...
        if (enableDeprecatedFlareAPI) { //废弃的事件监听的API
        ...
        if (wasHydrated) {
          ...
          // 某些元素dom支持自动聚焦 这些自动聚焦的过程在之后的过程中需要被处理
          if (
            finalizeInitialChildren(
              instance,
              type,
