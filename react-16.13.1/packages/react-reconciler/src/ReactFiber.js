...
// 是否是简单的function 模式组件 不含有defaultProps
...
// 创建一个备用的fiber节点(双缓存技术存在两颗fiber树 一个过去 一个现在。)能复用则复用否则创建新的fiber节点。主要是pendingProps更新
...
// 创建root fiber节点 参数为根节点的模式
export function createHostRootFiber(tag: RootTag): Fiber {
  let mode;
  if (tag === ConcurrentRoot) {//并发模式
...
// 根据react types 和 props 创建fiber节点
...
// 根据react element 元素创建fiber node 
export function createFiberFromElement(
  ...
  const pendingProps = element.props;//react Element新的属性
...
// 根据FundamentalComponent创建fiber节点
...
export function createFiberFromPortal(
  ...
  fiber.expirationTime = expirationTime;
  // portal的startNode比较特殊记录自己的外层容器DOM
  fiber.stateNode = {
    containerInfo: portal.containerInfo, //dom节点
