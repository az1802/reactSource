...
// 开发环境下利用invokeGuardedCallback包装运行可以willMount函数时捕获错误,这样就不会中断卸载
...
function commitNestedUnmounts(
  ...
  while (true) {
    ...
    while (node.sibling === null) { //兄弟节点处理完 返回父节点
...
function getHostSibling(fiber: Fiber): ?Instance {
  ...
  siblings: while (true) {
    ...
    while (node.sibling === null) { //没有兄弟节点 则向父级查找
...
function commitPlacement(finishedWork: Fiber): void {
  ...
  // children to find all the terminal nodes.
  if (isContainer) {
    // dom元素则将fiber对应的dom节点插入到parent下
    insertOrAppendPlacementNodeIntoContainer(finishedWork, before, parent);
  } else {
    // 组件渲染完成插入一个占位符的node节点
...
function insertOrAppendPlacementNodeIntoContainer(
  ...
  if (isHost || (enableFundamentalAPI && tag === FundamentalComponent)) {
    ...
    // 递归的操作fiber节点将其递归的添加到dom节点中 因为中间存在抽象层不具有自身的dom节点
    if (child !== null) {
...
function insertOrAppendPlacementNode(
  ...
  if (isHost || (enableFundamentalAPI && tag === FundamentalComponent)) {
    ...
    // 中间可能是抽象节点只是一层的透传不具有自己的dom层 所有向下递归查找子fiber节点   同时兄弟节点也是一起一样的处理
    const child = node.child;
    if (child !== null) {
...
function unmountHostComponents(
  ...
  while (true) {
    if (!currentParentIsValid) { //父节点是否合法
...
function commitDeletion(
  ...
  if (supportsMutation) {
    ...
    // 递归的移除子树 并且整个子树调用componentWillUnmount
    unmountHostComponents(finishedRoot, current, renderPriorityLevel);
  } else {
    // Detach refs and call componentWillUnmount() on the whole subtree.
